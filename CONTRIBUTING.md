...
* Find the definition of BusinessEntity in this file; it looks like this: 
```<owl:Class rdf:about="&fibo-be-le-lp;BusinessEntity">```

* A few lines later, notice a line that contains 
```&fibo-fnd-rel-rel;holds```.```

* Change the restriction ```owl:onClass rdf:resource="&fibo-fnd-law-lcap;License``` to ```owl:someValuesFrom rdf:resource="&fibo-fnd-law-lcap;License```, and remove the min 0 cardinality constraint on the following line. Save the file. 
...
* Examine LegalPersons.rdf,  It should have changed since you saved in a few steps ago.  Verify that the restriction has been revised accordingly.
